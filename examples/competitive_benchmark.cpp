...
static void print_result(const char* label, Stats& tp, Stats& lat) {
    std::printf("  %-28s Throughput: %8.2f ± %.2f M/s   Latency: P50=%6.0f P95=%6.0f P99=%6.0f ns\n",
                label, tp.mean, tp.stddev, lat.p50, lat.p95, lat.p99);
}
...
static void entt_handler(TestMsg&) { ++g_entt_count; }
...
static void bench_sigslot() {
    ...
    std::atomic<uint64_t> processed{0};
    sig.connect([&processed](const TestMsg&) {
        processed.fetch_add(1, std::memory_order_relaxed);
    });
    ...
    for (uint32_t r = 0; r < ROUNDS; ++r) {
        ...
        std::thread consumer([&]() {
            ...
            while (!done.load(std::memory_order_acquire) || consumed.load(std::memory_order_relaxed) < BENCH_MSGS) {
                ...
                if (result) consumed.fetch_add(1, std::memory_order_relaxed);
            }
        });
...
static void bench_zeromq_size(const char* label) {
    ...
    for (uint32_t r = 0; r < ROUNDS; ++r) {
        ...
        while (consumed.load(std::memory_order_relaxed) < BENCH_MSGS) std::this_thread::yield();
        consumer.join();
...
int main() {
    ...
    std::printf("\n[Warmup] Running warmup rounds...\n");
    {
        ...
        for (int i = 0; i < 100000; ++i) sig(TestMsg{0, 0, 0, 0, 0});
    }
