...
TEST_CASE("ProcessBatchWith dispatches all types", "[Visitor]") {
  ...
  auto visitor = mccc::make_overloaded(
      [&](const MsgA& m) { ++a_count; a_sum += m.value; },
      [&](const MsgB& m) { ++b_count; (void)m; },
      [&](const MsgC& m) { ++c_count; (void)m; });
...
TEST_CASE("ProcessBatchWith returns 0 on empty queue", "[Visitor]") {
  ...
  auto drain = mccc::make_overloaded(
      [](const MsgA&) {},
      [](const MsgB&) {},
      [](const MsgC&) {});
...
TEST_CASE("ProcessBatchWith throughput vs ProcessBatch", "[Visitor][benchmark]") {
  ...
  for (uint32_t i = 0; i < N; ++i) {
    bus.Publish(MsgA{static_cast<int>(i)}, 1);
  }

  auto count_visitor = mccc::make_overloaded(
      [&visitor_count](const MsgA&) { visitor_count.fetch_add(1U, std::memory_order_relaxed); },
      [](const MsgB&) {},
      [](const MsgC&) {});

  auto t1_start = std::chrono::steady_clock::now();
  while (visitor_count.load(std::memory_order_relaxed) < N) {
  ...
  while (callback_count.load(std::memory_order_relaxed) < N) {
    bus.ProcessBatch();
  }
  ...
  WARN("Visitor: " << visitor_ns / N << " ns/msg, Callback: " << callback_ns / N << " ns/msg");
  REQUIRE(visitor_count.load() == N);
  REQUIRE(callback_count.load() == N);
}
